...
BattleCommand_PSS:
; Dark, Ghost and Waterfall checks
...
.CheckHitmonchan:
	...
	cp WHT_APRICORN		;ToDo: Cambiar por el objeto final cuando se cree
	ret nz

	ld a, BATTLE_VARS_MOVE_ANIM
	call GetBattleVar
	ld de, 1
	ld hl, PhysicalPunches
	push bc
	call IsInArray
	pop bc
	ret nc
...
INCLUDE "data/moves/hitmonchan_punch_moves.asm"
...
BattleCommand_ItemStatus:
	; Only 30% of the times
	ld a, 10
...
BattleCommand_CheckSkipRecharge:
	ldh a, [hBattleTurn]
	and a
	ld hl, wEnemyMonItem
	jr nz, .Item
...
; Don't factor in time of day in link battles or in gyms.
	ld a, [wLinkMode]
	and a
	jr nz, .Weather
