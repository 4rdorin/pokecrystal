...
.checkpoint
	...
	cp DEXENTRY_AREA_GRASS_SWARM_ALT
	jr z, .grass ; _swarm
	...
	cp DEXENTRY_AREA_SURF_SWARM_ALT
	jr z, .surf
...
.none
	...
	jmp Pokedex_PrintPageNum
.found
	ld [wPokedexEntryType], a
	xor a
	ld [wPokedexStatus], a ; wildmon entry index
	ld [wPokedexEntryPageNum], a ; page num
	jmp Pokedex_DetailedArea
...
; grass, grass swarm, contest, trees(+rocks), surf, surf swarm, rods
	...
	jmp z, .grass_swarm
	ld hl, SwarmGrassWildMonsAlt
	ld a, BANK(SwarmGrassWildMonsAlt)
	call Dex_Check_Grass
	and a
	jmp z, .grass_swarm_alt
	...
	ld hl, SwarmWaterWildMonsAlt
	ld a, BANK(SwarmWaterWildMonsAlt)
	call Dex_Check_Surf
	and a
	jr z, .surf_swarm_alt
...
.grass_swarm
	...
	ret
.grass_swarm_alt
	ld a, DEXENTRY_AREA_GRASS_SWARM_ALT
	ret
.surf_johto
...
.surf_swarm
	...
	ret
.surf_swarm_alt
	ld a, DEXENTRY_AREA_SURF_SWARM_ALT
	ret
.rods
