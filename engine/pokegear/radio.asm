...
.generate_flag
	ld a, 2
	call RandomRange ; generate a random number between 0 and 2
	and a
...
.generate_number
	...
	ld hl, wSwarmFlags
	set SWARMFLAGS_SWARM, [hl]
	res SWARMFLAGS_SWARM_ALT, [hl]
	call .InJohto
	jp nc, .generate_number_kanto
	ld a, NUM_JOHTO_SWARMS
...
.generate_alt_number
	...
	set DAILYFLAGS1_SWARM_F, [hl]
	ld hl, wSwarmFlags
	set SWARMFLAGS_SWARM_ALT, [hl]
	res SWARMFLAGS_SWARM, [hl]
	call .InJohto
	jp nc, .generate_alt_number_kanto
	ld a, NUM_JOHTO_ALT_SWARMS
	call RandomRange ; genrate a random number between 0 and a
